...
export type WalkAloneSubStatus =
  | 'TRACKING'
  | 'PROMPT_SENT'
  | 'RESPONDED'
  | 'NON_RESPONSE'
  | 'ESCALATED';
export type SOSSubStatus =
  | 'BROADCASTING'
  | 'ESCALATED_PUSH'
  | 'ESCALATED_SMS'
  | 'ESCALATED_CALL';
...
// ─── Session Policies (type-specific config stored as JSONB) ──────────────────
...
export interface CheckInPolicy {
  ...
  shareWith: string[]; // trusted_contact UUIDs
}
...
export interface SafeRidePolicy {
  ...
  deviationThresholdMeters: number; // default 200
  stopThresholdSeconds: number; // default 120
  shareWith: string[];
}

export interface WalkAlonePolicy {
  type: 'WALKALONE';
  promptIntervalSeconds: number; // default 300
  shareWith: string[];
}

export interface SOSPolicy {
  type: 'SOS';
  shareWith: string[]; // all trusted contacts if empty
  smsDelaySeconds: number; // 30
  callDelaySeconds: number; // 60
}

export type SessionPolicy = CheckInPolicy | SafeRidePolicy | WalkAlonePolicy | SOSPolicy;

// ─── Session Details (1:1 type-specific DB rows) ──────────────────────────────
...
export function isCheckIn(s: SafetySession): s is CheckInSession {
  return s.type === 'CHECKIN';
}
export function isSafeRide(s: SafetySession): s is SafeRideSession {
  return s.type === 'SAFERIDE';
}
export function isWalkAlone(s: SafetySession): s is WalkAloneSession {
  return s.type === 'WALKALONE';
}
export function isSOS(s: SafetySession): s is SOSSession {
  return s.type === 'SOS';
}
