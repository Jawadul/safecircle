...
describe('haversineDistance', () => {
    it('returns 0 for identical points', () => {
        expect((0, geo_1.haversineDistance)({ lat: 23.8, lng: 90.4 }, { lat: 23.8, lng: 90.4 })).toBe(0);
    });
    it('returns ~111km per degree latitude', () => {
        const d = (0, geo_1.haversineDistance)({ lat: 0, lng: 0 }, { lat: 1, lng: 0 });
    ...
    it('returns correct distance for Dhaka landmarks', () => {
        ...
        const d = (0, geo_1.haversineDistance)(airport, gulshan);
...
describe('decodePolyline', () => {
    it('decodes a known Google polyline', () => {
        ...
        const pts = (0, geo_1.decodePolyline)(encoded);
...
describe('pointToPolylineDistance', () => {
    it('returns 0 for a point on the polyline', () => {
        ...
        const d = (0, geo_1.pointToPolylineDistance)({ lat: 0, lng: 0.5 }, line);
        expect(d).toBeCloseTo(0, 0);
    });
    it('returns Infinity for empty polyline', () => {
        expect((0, geo_1.pointToPolylineDistance)({ lat: 0, lng: 0 }, [])).toBe(Infinity);
    });
    it('detects a point significantly off route', () => {
        ...
        const d = (0, geo_1.pointToPolylineDistance)(offRoute, line);
...
describe('distanceFromRoute', () => {
    it('accepts encoded polyline string', () => {
        ...
        const d = (0, geo_1.distanceFromRoute)(point, encoded);
        expect(d).toBeCloseTo(0, 0);
    });
});
describe('encodeGeohash', () => {
    it('encodes known location to correct geohash prefix', () => {
        ...
        const hash = (0, geo_1.encodeGeohash)({ lat: 23.8103, lng: 90.4125 }, 6);
        expect(hash).toHaveLength(6);
        expect(typeof hash).toBe('string');
    });
    it('returns same hash for same point', () => {
        const p = { lat: 23.8103, lng: 90.4125 };
        expect((0, geo_1.encodeGeohash)(p, 6)).toBe((0, geo_1.encodeGeohash)(p, 6));
    });
});
describe('speedBetween', () => {
    it('returns null for zero time delta', () => {
        const t = '2024-01-01T00:00:00.000Z';
        expect((0, geo_1.speedBetween)({ lat: 0, lng: 0, timestamp: t }, { lat: 1, lng: 0, timestamp: t })).toBeNull();
    });
    it('calculates correct speed for stationary point', () => {
        const speed = (0, geo_1.speedBetween)({ lat: 23.8, lng: 90.4, timestamp: '2024-01-01T00:00:00.000Z' }, { lat: 23.8, lng: 90.4, timestamp: '2024-01-01T00:01:00.000Z' });
